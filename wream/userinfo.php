...
$user_id_mysql=mysql_query("select user_id from users where username='$username'");
$user_id_show=mysql_fetch_assoc($user_id_mysql);
$user_id=$user_id_show['user_id'];

$userimg_mysql=mysql_query("select userimg from users where user_id='$user_id'");
$userbg_mysql=mysql_query("select userbg from users where user_id='$user_id'");

$userimg=mysql_fetch_array($userimg_mysql);
$userbg=mysql_fetch_array($userbg_mysql);
$userbg_control=mysql_num_rows($userbg_mysql);

$userpost=mysql_query("select * from posts where post_author='$username'");
$userpost_control=mysql_num_rows($userpost);
?>
<center>
	<div class="demo_head" style="background: url('<?php echo $userbg['userbg'] ?>');background-repeat: no-repeat;background-size:cover;"><a href="<?php echo $userimg['userimg'];?>"><img src="<?php echo $userimg['userimg'];?>" style="border-radius: 100%;max-width: 150px;height: 150px"></a><h1><?php echo $username; ?></h1>
	</div>
</center>
<div id="blog-section">
	<?php
		...
		<div class="row">
			<div class="col-xs-12 col-md-8">
				...
				<div style="background-color: white;height: 800px;border-radius: 5px">
					<center><h3>User did not share anything.</h3><hr>
						<img src="images/empty.png" style="width: 200px"><i><h4>Empty user profile</h4></i>
					</center>
				</div>
				<br>
				<?php
